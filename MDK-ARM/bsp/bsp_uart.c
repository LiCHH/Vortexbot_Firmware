...
static void uart_rx_idle_callback(UART_HandleTypeDef *huart)
{
  ...
  /* handle recieve data in idle interrupt */
  if (huart == &TEST_HUART)
  {
    /* clear DMA transfer complete flag */
    __HAL_DMA_DISABLE(huart->hdmarx);
    // 在DMA Normal模式下，每次接收完成后要清除DMA标志，否则无法再接收， 在Circular模式下则不用
    // 清除DMA对应流的TCIF(传输完成标志)
    __HAL_DMA_CLEAR_FLAG(huart->hdmarx, __HAL_DMA_GET_TC_FLAG_INDEX(huart->hdmarx));

    if (1)
    {
      testctrl_callback_handler(&tc, test_buf);
    }
...
//   UART_Receive_DMA_No_IT(&TEST_HUART, dbus_buf, DBUS_MAX_LEN);
// }
