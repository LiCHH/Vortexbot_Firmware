...
static void omnidirection_handler(void)
{
  ...
  if (!(fabs(chassis.vx) < FLOAT_THRESHOLD && fabs(chassis.vy) < FLOAT_THRESHOLD)) {
    ...
    // sprintf(test_buf, "%d %d\r\n", chassis.driving_spd_ref[0], chassis.driving_spd_ref[1]);
    // HAL_UART_Transmit_DMA(&TEST_HUART, (uint8_t *)test_buf, 15);
...
void versatile_control(float vx, float vy, float vw, float* omegas, float* phis) {
  ...
  vxs[fr_motor] = vx + BODY_RADIUS * vw;
  vys[fr_motor] = vy + BODY_RADIUS * vw;
  vxs[fl_motor] = vx - BODY_RADIUS * vw;
  vys[fl_motor] = vy + BODY_RADIUS * vw;
  vxs[bl_motor] = vx - BODY_RADIUS * vw;
  vys[bl_motor] = vy - BODY_RADIUS * vw;
  vxs[br_motor] = vx + BODY_RADIUS * vw;
  vys[br_motor] = vy - BODY_RADIUS * vw;
